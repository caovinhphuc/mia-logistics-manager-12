...
// POST /api/telegram/test -> gửi tin nhắn test (hỗ trợ POST method)
router.post('/test', async (req, res) => {
  try {
    ...
    // Cho phép custom message từ body, nếu không có thì dùng message mặc định
    const customMessage = req.body?.message || req.body?.text;
    const text = customMessage
      ? `✅ ${customMessage}\n⏰ ${new Date().toISOString()}`
      : `✅ Kiểm tra kết nối Telegram thành công: ${new Date().toISOString()}`;
    const result = await sendTelegramMessage({ token, chatId, text });
    return res.json({ success: true, result });
  } catch (err) {
    return res.status(err.status || 500).json({
...
router.get('/env', (req, res) => {
  ...
  res.json({
    ...
    chatIdPreview: chatId
      ? `${String(chatId).slice(0, 4)}…(${String(chatId).length})`
      : null,
